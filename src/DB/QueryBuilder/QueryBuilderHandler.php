...
use WPDrill\DB\Connection;
use WPDrill\DB\Exception;
...
class QueryBuilderHandler
{
    ...
    public function query($sql, $bindings = array())
    {
        $this->dbStatement = $this->container->build(
            '\\WPDrill\DB\\QueryBuilder\\QueryObject',
            array($sql, $bindings)
        )->getRawSql();
    ...
    public function join($table, $key, $operator = null, $value = null, $type = 'inner')
    {
        ...
        $joinBuilder = $this->container->build('\\WPDrill\DB\\QueryBuilder\\JoinBuilder', array($this->connection));
    ...
    public function transaction(\Closure $callback)
    {
        try {
            ...
            $transaction = $this->container->build(
                '\\WPDrill\DB\\QueryBuilder\\Transaction',
                array($this->connection)
            );
    ...
    {
        return $this->container->build('\\WPDrill\DB\\QueryBuilder\\Raw', array($value, $bindings));
    }
